...
# Directories
hovmollerdir <- "output/hovmoller"
eventsdir <- "output/contam_events"
...
for(i in 1:length(best_models)){
  ...
  load(file.path(hovmollerdir, infile))
  rm(data_df)
  ...
  if(i!=4){
    ...
    data_imputed <- data_stats %>% 
      ...
      mutate("36.10"=NA,
             ...
             "36.19"=NA) %>% 
      # Arrange columns
      dplyr::select(date, "36.07", "36.10", "36.13", "36.16", "36.19", "36.22") %>% 
      ...
      dplyr::select(date, lat2, lat4, lat5) %>% 
      ...
      dplyr::select(date, lat_dd, n, pcontam_avg)
    ...
    data_imputed <- data_stats %>% 
      ...
      mutate("39.94"=NA) %>% 
      # Arrange columns
      dplyr::select(date, "39.91", "39.94", "39.97") %>% 
      rename("lat1"="39.91", "lat2"="39.94", "lat3"="39.97") %>% 
      # Perform interpolation
      mutate(lat2 = lat1 + (lat3-lat1)/2) %>% 
      # Reduce to interpolated data to be appended to data frame
      dplyr::select(date, lat2) %>% 
      gather(key="lat_dd", value="pcontam_avg", 2:ncol(.)) %>% 
      mutate(n=NA,
  ...
  save(data_hov_imputed, events_ras_df, events_stats, 
       file=file.path(eventsdir, outfile))
