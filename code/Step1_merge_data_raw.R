...
datadir_cdph <- "data/da_sampling/2020_request/processed"
inputdir <- "input"
tabledir <- "tables"
plotdir <- "figures"

# Read data
crabs_orig <- readRDS(file.path(datadir_cdph, "CDPH_crab_viscera_da_data.rds"))
bivalves_orig <- readRDS(file.path(datadir_cdph, "CDPH_mollusc_viscera_da_data.rds"))
...
# Read C-HARM data
pn_brick <- raster::brick(file.path(datadir_charm, "CHARM_DAP_20140305_to_present_imputed.grd"))
...
# Export data
saveRDS(data2, file.path(inputdir, "CDPH_crab_bivalve_domoic_acid_data.Rds"))


# Inspect sample size of raw data
################################################################################
